...
const BOARD_CONFIG = {
  ...
  SYMBOLS: { k: '♚', q: '♛', r: '♜', b: '♝', n: '♞', p: '♟', wine_knight: '♞', cyber_drone: '♟' } as const
};

const PieceRenderer = memo(({ cell }: { cell: Cell }) => (
  <div className={`relative w-10 h-10 sm:w-14 sm:h-14 rounded-full flex items-center justify-center transition-all duration-300 ${cell.faction === 'jesus' ? 'text-white bg-slate-950/40 border border-white/20' : 'text-slate-950 bg-slate-100 border-2'}`}>
    ...
    <span className="text-2xl sm:text-4xl z-10">{BOARD_CONFIG.SYMBOLS[cell.type as keyof typeof BOARD_CONFIG.SYMBOLS] || ''}</span>
  </div>
));
        ...
        {board.map((rowArr, r) => rowArr.map((cell, c) => {
          ...
          return (
            <div 
              ...
              className={`relative aspect-square flex items-center justify-center select-none group transition-all ${isDark ? 'bg-[#0b0f19]' : 'bg-[#182030]'} ${isSelected ? 'ring-4 ring-inset ring-amber-500/80 bg-amber-950/20' : ''} ${isValid ? 'cursor-pointer hover:bg-blue-900/20' : ''}`}>
              {c === 0 && <span className="absolute top-1 left-1 text-[8px] text-slate-500/60 font-mono font-bold">{BOARD_CONFIG.ROWS[r]}</span>}
              {r === 7 && <span className="absolute bottom-1 right-1 text-[8px] text-slate-500/60 font-mono font-bold">{BOARD_CONFIG.COLS[c]}</span>}
